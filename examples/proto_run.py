...
from data_cal_proto import sigperf
...
RET = get_stk_return('20100101', '20200105')                                # 获取股票收益率


# ----------------------------- 数据更新 --------------------------
# 声明数据计算函数所在的脚本，数据存储方式，数据存储链接方式，数据基期(数据创建后数据开始更新日期，只应用于range更新方式)
update_instance = DataUpdate.indicator_from_func(func=sigperf, table_name='signal_ic', storage_type="sql", conn=conn, base_date='20100101', ret=RET)
# 更新交易日更新的数据
